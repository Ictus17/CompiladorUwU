...
public class Expansion {
    ...
    public static Image expansionCuadratica(Image imagenOriginal){
        BufferedImage bi = ImageManager.toBufferedImage(imagenOriginal);
        // recorremos la imagen 
        for(int x=0; x<bi.getWidth();x++){
            for (int y=0;y<bi.getHeight();y++){
            ...
            int r = (int)validarRango(Math.pow(color.getRed(),2));
            int g = (int)validarRango(Math.pow(color.getGreen(),2));
            int b = (int)validarRango(Math.pow(color.getBlue(),2));
